...
public class VideoView extends SurfaceView implements
    ...
    public static final int VIDEO_LAYOUT_FULL_SCREEN = 4;    
    ...
    private boolean mAutoShowMediaControl = true;
    ...
    public void setVideoLayout(int layout) {
        ...
        if(mSelfHeight == 0 || mSelfWidth == 0 || mSelfHeight == mVideoHeight || mSelfWidth == mVideoWidth){
        	mSelfWidth = getWidth();
        ...
        if (mVideoHeight > 0 && mVideoWidth > 0) {
            ...
            if (VIDEO_LAYOUT_ORIGIN == layout && mSurfaceWidth < windowWidth
                ...
                lp.height = windowRatio < videoRatio ? windowWidth
                        : (int) (windowWidth / videoRatio);
                lp.setMargins(0, 0, 0, 0);
            } else if (layout == VIDEO_LAYOUT_STRETCH) {
                lp.width = mSelfWidth;
                lp.height = mSelfHeight;
                lp.setMargins(0, 0, 0, 0);
            } else if (layout == VIDEO_LAYOUT_FULL_SCREEN) {
            	lp.width = windowWidth;
            	lp.height = windowHeight;
            	lp.setMargins(0, 0, 0, 0);
            } else {
            	int marginlr,margintb;
            	
            	int videomustw = (int) (mSelfHeight * videoRatio);
            	
            	if(videomustw > mSelfWidth){
            		int videomusth = (int) (mSelfWidth / videoRatio);
            		marginlr = 0;
            		lp.width = mSelfWidth;
            		lp.height = videomusth;
            		margintb = (mSelfHeight - videomusth)/2;
            	}else{
            		margintb = 0;
            		lp.width = videomustw;
            		lp.height = mSelfHeight;
            		marginlr = (mSelfWidth - videomustw)/2;
            	}
...
//        setFocusableInTouchMode(true);
    ...
    OnPreparedListener mPreparedListener = new OnPreparedListener() {
        public void onPrepared(IMediaPlayer mp) {
            ...
            if(mAutoPlay || mTargetState == STATE_PLAYING)
    ...
    SurfaceHolder.Callback mSHCallback = new SurfaceHolder.Callback() {
        public void surfaceChanged(SurfaceHolder holder, int format, int w,
            ...
            if (mMediaPlayer != null && isValidState && hasValidSize) {
                ...
                if (mMediaController != null && mAutoShowMediaControl) {
	...
	public boolean isAutoShowMediaControl() {
		return mAutoShowMediaControl;
	}

	public void setAutoShowMediaControl(boolean mAutoShowMediaControl) {
		this.mAutoShowMediaControl = mAutoShowMediaControl;
	}
